...
def create_cluster_output(cluster_df, homologous_seqs, output_tar_file):
    ...
    for i, cluster_id in enumerate(cluster_df["repID"].unique()):
        ...
        output_file = os.path.join(
            "data", "foldseek_cluster", f"cluster_{cluster_id}.faa"
        )
        ...
        with open(output_file, "w", newline="\n") as f:
            ...
            for h_seq in homologous_seqs:
                if (
                    h_seq
                    in cluster_df[cluster_df["repID"] == cluster_id]["memID"].unique()
                ):
                    SeqIO.write(homologous_seqs[h_seq], f, "fasta")

            # then get sequences for other clusters
            n_members = 0
            for member_ids in cluster_df[cluster_df["repID"] == cluster_id][
                "memID"
            ].unique():
...
def get_fasta_from_uniprot(uniprot_id):
    ...
    if response.status_code == 200:
        ...
        print(
            f"Error: Unable to fetch data for {uniprot_id}. Status code: {response.status_code}"
        )
        return None
...
if __name__ == "__main__":
    # load sequences
    homologous_seqs = {
        x.id.split("|")[1]: x for x in list(SeqIO.parse(snakemake.input[0], "fasta"))
    }
    print(
        f"Detected the following Rhea homologous proteins: {list(homologous_seqs.keys())}"
    )
    ...
    foldseek_clusters = foldseek_clusters[
        (~foldseek_clusters["cluFlag"] != 3) & (~foldseek_clusters["cluFlag"] != 4)
    ]
    print(
        f"Loaded Foldseek cluster database containing {len(foldseek_clusters)} proteins in {foldseek_clusters['repID'].nunique()} non-singleton clusters!"
    )
    ...
    print(
        f"Identified {len(cluster_ids)} non-singleton clusters, extracting cluster information..."
    )
    ...
    print(
        f"Found {len(seq_clusters)} structually similar proteins! Retrieving fasta sequences..."
    )
