...
if __name__ == "__main__":
    ...
    if FILTER_BY_SPROT:
        ...
        with gzip.open(sprot_fasta, "rt") as f:
            swissprot_accessions = set(record.id.split("|")[1] for record in SeqIO.parse(f, "fasta"))
    else:
        print("Building database with ALL FoldSeek entries... (this will take a long time)")
    ...
    for chunk in tqdm(pd.read_csv(foldseek_clusters_file, names=["repId", "memId", "cluFlag", "taxId"], sep="\t", chunksize=CHUNK_SIZE, compression="gzip"), total=n_chunks, desc="Building cluster database", unit="chunk"):
    ...
    if FILTER_BY_SPROT:
        print(f"Found clusters for {row_count}/{len(swissprot_accessions)} ({row_count / len(swissprot_accessions):.1%}) SwissProt accessions")
    else:
